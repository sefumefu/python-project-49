# Brain Games

Набор игр для тренировки мозга: арифметика, чётные числа, НОД, простые числа и прогрессии.

## Минимальные требования

- Python >= 3.13
- uv

## Установка

```bash
uv tool install git+https://github.com/sefumefu/python-project-49
```

## Запуск

```bash
brain-games
brain-calc
brain-even
brain-gcd
brain-prime
brain-progression
```

## Code quality

[![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=sefumefu_python-project-49&metric=alert_status)](https://sonarcloud.io/summary/new_code?id=sefumefu_python-project-49)
[![Bugs](https://sonarcloud.io/api/project_badges/measure?project=sefumefu_python-project-49&metric=bugs)](https://sonarcloud.io/summary/new_code?id=sefumefu_python-project-49)
[![Code Smells](https://sonarcloud.io/api/project_badges/measure?project=sefumefu_python-project-49&metric=code_smells)](https://sonarcloud.io/summary/new_code?id=sefumefu_python-project-49)

## Hexlet tests and linter status

[![Actions Status](https://github.com/sefumefu/python-project-49/actions/workflows/hexlet-check.yml/badge.svg)](https://github.com/sefumefu/python-project-49/actions)

## Demo

### brain-games
[![asciicast](https://asciinema.org/a/PH9URqLjbhAfxx3t.svg)](https://asciinema.org/a/PH9URqLjbhAfxx3t)

### brain-even
[![asciicast](https://asciinema.org/a/uXcpyLqDaLlJ2Jdc.svg)](https://asciinema.org/a/uXcpyLqDaLlJ2Jdc)

### brain-calc
[![asciicast](https://asciinema.org/a/2ryEMISQkSJOOI0j.svg)](https://asciinema.org/a/2ryEMISQkSJOOI0j)

### brain-gcd
[![asciicast](https://asciinema.org/a/hlfvv5CEube4xlGx.svg)](https://asciinema.org/a/hlfvv5CEube4xlGx)

### brain-prime
[![asciicast](https://asciinema.org/a/8M4TEaOMT594QCas.svg)](https://asciinema.org/a/8M4TEaOMT594QCas)
